; Zed highlight query — converted from ../highlights.scm (the Helix one).
;
; Capture names follow Zed's documented scope set
; (https://zed.dev/docs/extensions/languages#syntax-highlighting):
;   @keyword.control* / @keyword.operator  -> @keyword
;   @keyword.directive                     -> @preproc
;   @comment.line / @comment.block         -> @comment   (+ @comment.doc)
...
;   @type.parameter                        -> @type
;   @namespace                             -> @type @namespace   (Zed resolves
;     multi-captures right-to-left: themes that style `namespace` use it,
;     the rest fall back to `type`.)
;
; Pattern ORDER is meaningful and mirrors the Helix file: when several
; patterns capture the same node, the last capture in source order wins
; (Zed resolves like Helix here). Keep the two files' rule order in sync.
...
; Re-capture with a non-themed name so it wins on
